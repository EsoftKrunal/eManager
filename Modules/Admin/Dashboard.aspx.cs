...
public partial class Modules_Admin_Dashboard : System.Web.UI.Page
{
    ...
    {
        string strSQL = "SELECT ModuleId,ModuleName,MenuIcon,PURL,MenuIcon  FROM DBO.Appmstr_Module with(nolock) WHERE IsActive = 1 and ModuleName not in ( 'MASTERS','ADMIN', 'ANALYTIC')  order by DisOrder ASC";
...

            {
...

                {
                    ...
                    if (dbcount < total)
                    {
                        ...
                        string imageURL = dtModules.Rows[dbcount]["MenuIcon"].ToString();
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            String sql = "Select PageUrl from appmstr_Page p with(nolock) Inner Join appmstr_Menu m with(nolock) on P.PageID = m.PageID inner join appmstr_Module mo with(nolock) on m.MenuID  = mo.DefaultMenuId where mo.ModuleName = '" + Imgbtn.ToolTip + "'";
