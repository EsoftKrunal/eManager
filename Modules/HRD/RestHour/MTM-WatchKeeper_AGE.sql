...
GO
/****** Object:  View [dbo].[VW_CP_NCLIST]    Script Date: 06/15/2012 16:39:14 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VW_CP_NCLIST]'))
DROP VIEW [dbo].[VW_CP_NCLIST]
GO
/****** Object:  Table [dbo].[CP_NCReason]    Script Date: 06/15/2012 16:39:12 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[CP_NCReason]') AND type in (N'U'))
DROP TABLE [dbo].[CP_NCReason]
GO
...
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[CP_NCReason]') AND type in (N'U'))
BEGIN
CREATE TABLE [dbo].[CP_NCReason](
	[NCReasonId] [int] NOT NULL,
	[NCReasonName] [varchar](100) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
 CONSTRAINT [PK_CP_NCReason] PRIMARY KEY CLUSTERED 
(
	[NCReasonId] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON)
)
...
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VW_CP_NCLIST]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW [dbo].[VW_CP_NCLIST]
AS
SELECT     M.NCDate, dbo.CP_NCType.NCTypeName,
                          (SELECT     NCReasonName
                            FROM          dbo.CP_NCReason AS N
                            WHERE      (NCReasonId IN
                                                       ((SELECT     Reason
                                                           FROM         dbo.CP_NonConformanceReason AS D
                                                           WHERE     (CrewNumber = M.CrewNumber) AND (NCDate = M.NCDate))))) AS REASON, M.CrewNumber
FROM         dbo.CP_NonConformance AS M INNER JOIN
                      dbo.CP_NCType ON dbo.CP_NCType.NCTypeId = M.NCTYPE
'
GO
...
END
------------------------------------ GENERATING DATA FOR -1 MONTH -6 TO +1 MONTH +6 DAYS ON THE CALC DATE
DECLARE @CNT INT
...
SET @TEMPDT=DATEADD(DAY,-6,@TEMPDT)
SET @ENDDT=DATEADD(DAY,6,@ENDDT)
WHILE(@TEMPDT<=@ENDDT)
BEGIN
...
BEGIN
	...
	-- FIRSTDAY
	--	SELECT @PARTFIRST=ISNULL(SUM(DURATION),0) FROM CP_CrewDailyWorkRestHours WHERE WORKREST=''R'' AND VESSELID=@VESSELID AND CREWID=@CREWID AND TRANSDATE=@TEMPDT AND FROMTIME >=@STTIME
	--	-- MIDDLEDAYS
	--	SELECT @PARTMIDDLE=@PARTMIDDLE+ISNULL(SUM(DURATION),0) FROM CP_CrewDailyWorkRestHours WHERE WORKREST=''R'' AND VESSELID=@VESSELID AND CREWID=@CREWID AND TRANSDATE > @TEMPDT AND TRANSDATE< DATEADD(DAY,6,@TEMPDT)
	--	-- LASTDAY
	--	SELECT @PARTLAST=ISNULL(SUM(CASE WHEN TOTIME < @ENDTIME THEN DURATION ELSE @ENDTIME-FROMTIME+.5 END),0) FROM CP_CrewDailyWorkRestHours WHERE WORKREST=''R'' AND VESSELID=@VESSELID AND CREWID=@CREWID AND TRANSDATE=DATEADD(DAY,6,@TEMPDT) AND FROMTIME < @ENDTIME
...
--		DECLARE @D1 SMALLDATETIME 
--		SET @D1=@TEMPDT
--		WHILE(@D1 <=DATEADD(DAY,6,@TEMPDT) )
--		BEGIN
--			IF NOT EXISTS(SELECT * FROM CP_NonConformance WHERE VESSELID=@VESSELID AND CREWID=@CREWID AND NCDATE=@D1 AND CALCDATE=@ONDATE AND NCTYPE=7 AND COMPTIME=@COMPTIME)
--			INSERT INTO CP_NonConformance VALUES(@VESSELID,@CREWID,@D1,@ONDATE,7,@COMPTIME)
...
--		END 
	...
	BEGIN
		-- FIRSTDAY
		SELECT @PARTFIRST=ISNULL(SUM(DURATION),0) FROM CP_CrewDailyWorkRestHours WHERE WORKREST=''R'' AND CREWNUMBER=@CREWNUMBER AND TRANSDATE=@TEMPDT AND FROMTIME >=@STTIME
		-- LASTDAY
		IF @STTIME<>0.0 
			SELECT @PARTLAST=ISNULL(SUM(CASE WHEN TOTIME < @ENDTIME THEN DURATION ELSE @ENDTIME-FROMTIME+.5 END),0) FROM CP_CrewDailyWorkRestHours WHERE WORKREST=''R'' AND CREWNUMBER=@CREWNUMBER AND TRANSDATE=dateadd(day,1,@TEMPDT) AND FROMTIME <=@ENDTIME
		----------------
		UPDATE CP_CrewDailyWorkRestHours SET RESTIN24=@PARTFIRST+@PARTLAST WHERE WORKREST=''W'' AND CREWNUMBER=@CREWNUMBER AND TRANSDATE=@TEMPDT AND FROMTIME=@STTIME 
...
--		SELECT ISNULL(DURATION,0),1 FROM CP_CrewDailyWorkRestHours 
--		WHERE WORKREST=''R'' AND VESSELID=@VESSELID AND CREWID=@CREWID AND TRANSDATE=@TEMPDT AND FROMTIME >=@STTIME
--		-- LASTDAY
--		IF @STTIME<>0.0 
--		BEGIN
--			INSERT INTO @DURATIONS 
--			SELECT ISNULL(CASE WHEN TOTIME < @ENDTIME THEN DURATION ELSE @ENDTIME-FROMTIME+.5 END,0),2 FROM CP_CrewDailyWorkRestHours 
--			WHERE WORKREST=''R'' AND VESSELID=@VESSELID AND CREWID=@CREWID AND TRANSDATE=dateadd(day,1,@TEMPDT) AND FROMTIME <=@STTIME
--		END
...
/****** Object:  Table [dbo].[CP_Settings]    Script Date: 06/15/2012 16:44:23 ******/
INSERT [dbo].[CP_Settings] ([VesselCode], [VesselName], [Lrimonumber], [Flag], [AdminPassword]) VALUES (N'AGE', N'Agate V', N'9424534', N'SINGAPORE', N'vx/kiVeG9SLWq8U6azCFRg==')
/****** Object:  Table [dbo].[CP_NCReason]    Script Date: 06/15/2012 16:44:23 ******/
INSERT [dbo].[CP_NCReason] ([NCReasonId], [NCReasonName]) VALUES (1, N'Reason 1')
INSERT [dbo].[CP_NCReason] ([NCReasonId], [NCReasonName]) VALUES (2, N'Reason 2')
INSERT [dbo].[CP_NCReason] ([NCReasonId], [NCReasonName]) VALUES (3, N'ReasoN 3')
