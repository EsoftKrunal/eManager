...
public class getAutoCompleteData : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (type == "HAZARD")
        {
            ...
            DataTable dt = Common.Execute_Procedures_Select_ByQuery("SELECT HAZARDCODE + ' : ' + HAZARDNAME AS HNAME,HAZARDID FROM DBO.EV_HAZARDMASTER WHERE HAZARDNAME LIKE '" + Key + "%' ORDER BY HAZARDNAME ");
        ...
        else if (type == "CONSEQUENCES")
        {
            ...
            DataTable dt = Common.Execute_Procedures_Select_ByQuery("SELECT CONSEQUENCESCODE + ' : ' + CONSEQUENCESNAME AS CNAME,CONSEQUENCESID FROM DBO.RA_ConsequencesMaster WHERE CONSEQUENCESNAME LIKE '" + Key + "%' ORDER BY CONSEQUENCESNAME ");
            string json = JsonConvert.SerializeObject(dt, Formatting.None);
            context.Response.Write("{\"totalResultsCount\" : " + dt.Rows.Count.ToString() + ",\"geonames\" : " + json + " }");
        }
        else
