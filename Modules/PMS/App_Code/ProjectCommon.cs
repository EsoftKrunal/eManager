...
public class ProjectCommon
{
    ...
    public static MailAddress _FromAdd = new MailAddress("emanager@energiossolutions.com");
    public static string _UserName = "emanager@energiossolutions.com";
    public static string _Password = "Huz41628";
    ...
    {   
        ...
        try
        {
            ...
            SmtpClient objSmtpClient = new SmtpClient("192.168.1.7", 25);
    ...
    {
        DataTable dtuser = Common.Execute_Procedures_Select_ByQueryAdmin("select email from usermaster where loginid=" + LoginId);
        string UserEmail="";
        if(dtuser !=null)
    ...
    public static bool SendeMail_MTM_Async(int LoginId, string FromAddress, string ReplyToAddress, string[] ToAddress, string[] CCAddress, string[] BCCAddress, string Subject, string Message, out string Error, string AttachmentFilePath)
    {
        bool MailSend = false;
        try
        {
    ...
    public static float getROB(string VesselCode,int ComponentId,string OfficeShip,int SpareId,DateTime dtOnDate)
    {
        ...
        if(dt!=null)
            if (dt.Rows.Count > 0)
            {
                rob = (float)Common.CastAsDecimal(dt.Rows[0][0].ToString()); 
            }
        return rob;
    }
