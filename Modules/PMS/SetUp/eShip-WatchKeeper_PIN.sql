...
/****** Object:  Table [dbo].[CP_Settings]    Script Date: 06/15/2012 16:44:23 ******/
INSERT [dbo].[CP_Settings] ([VesselCode], [VesselName], [Lrimonumber], [Flag], [AdminPassword]) VALUES (N'PIN', N'RIPLEY PINNACLE', N'9291418', N'LIBERIA', N'vx/kiVeG9SLWq8U6azCFRg==')
/****** Object:  Table [dbo].[CP_NCReason]    Script Date: 06/15/2012 16:44:23 ******/
...
/****** Object:  Table [dbo].[VesselCrewList]    Script Date: 06/15/2012 16:39:12 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[VesselCrewList]') AND type in (N'U'))
DROP TABLE [dbo].[VesselCrewList]
GO
/****** Object:  Table [dbo].[VesselCrewList]    Script Date: 30-08-2023 09:16:52 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[VesselCrewList]') AND type in (N'U'))
BEGIN
Create table VesselCrewList (
 VesselCode char(3) NOT NULL, CrewId Bigint NOT NULL, ContractId Bigint NOT NULL, RankId Bigint NOT NULL, WatchKeeper Bit Default(0) Not NULL
 CONSTRAINT [PK_VesselCrewList] PRIMARY KEY CLUSTERED 
(
	[VesselCode] ASC,
	[CrewId] ASC,
	[ContractId] ASC
...
/****** Object:  StoredProcedure [dbo].[InsertUpdateVesselCrewList]    Script Date: 12-Jun-2015 09:18:37 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[InsertUpdateVesselCrewList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].InsertUpdateVesselCrewList
GO
/****** Object:  StoredProcedure [dbo].[InsertUpdateVesselCrewList]    Script Date: 1/18/2023 11:19:51 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[InsertUpdateVesselCrewList]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N' 
Create PRoc InsertUpdateVesselCrewList
@VesselCode char(3),
@CrewId Bigint,
@ContractId Bigint,
@RankId Int,
...
AS
BEGIN
If Exists (Select top 1 1 from VesselCrewList with(nolock) where Vesselcode = @VesselCode and Crewid = @CrewId And ContractId = @ContractId)
BEGIN
    Update VesselCrewList
	SET RankId = @RankId, WatchKeeper = @WatchKeeper where Vesselcode = @VesselCode and Crewid = @CrewId And ContractId = @ContractId
END 
ELSE
BEGIN
   Insert Into VesselCrewList (VesselCode
,CrewId
,ContractId
,RankId
,WatchKeeper) VALUES (@VesselCode,@CrewId,@ContractId,@RankId,@WatchKeeper)
END
END
