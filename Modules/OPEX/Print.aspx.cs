...
public partial class Print : System.Web.UI.Page
{
    ...
    public void ShowCrystalReportForRFQ2()
    {
        ...
        CrystalReportViewer1.ReportSource = rpt;
        //rpt.Load(Server.MapPath("~/Report/PrintRFQ.rpt"));
        rpt.Load(Server.MapPath("~/Report/PrintOfficeRFQ2.rpt"));
        DsRFQMaster.Tables[0].TableName = "sp_NewPR_getRFQMasterByBidId;1";
        DsRFQMaster.Tables[1].TableName = "sp_NewPR_getRFQDetailsByBidId;1";
        rpt.SetDataSource(DsRFQMaster);
        DataTable dt = Common.Execute_Procedures_Select_ByQuery("SELECT SHIPNAME FROM VW_tblSMDVessels WHERE SHIPID='" + DsRFQMaster.Tables[0].Rows[0]["SHIPID"].ToString() + "'");
    ...
    public void ShowCrystalReportForPO()
    {
        ...
        string company = "";
        string email = "";
        DataTable dt1 = Common.Execute_Procedures_Select_ByQuery("Select a.Address,a.[Company Name] As Company, a.ImageURL,a.Email from Accountcompany a with(nolock) Inner Join Vessel v with(nolock) on a.Company = v.AccontCompany where VesselCode ='" + DsRFQMaster.Tables[0].Rows[0]["SHIPID"].ToString() + "'");
        if (dt1.Rows.Count > 0)
        {
    ...
    public void ShowActivityReport()
    {
        int  Month;
        string Year;
        Month = Common.CastAsInt32(Page.Request.QueryString["Month"].ToString());
        ...
        rpt.SetParameterValue("MonYear", ProjectCommon.GetMonthName(Month.ToString()).ToUpper() + " " + Year.ToString());
    }
...
//	Response.End();
    ...
    {
        // Query String parameter 
        string Comp, vess, BType, StartDate, EndDate, Year, Days, CurrFinancialYr, PreFinYr;
        Comp = Page.Request.QueryString["Comp"].ToString();
        vess = Page.Request.QueryString["Vessel"].ToString();
        BType = Page.Request.QueryString["BType"].ToString();
        ...
        Year = Page.Request.QueryString["year"].ToString();
        Days = Page.Request.QueryString["YearDays"].ToString();
        CurrFinancialYr = Page.Request.QueryString["CurrFinYr"].ToString();
        PreFinYr = Page.Request.QueryString["PreFinYr"].ToString();
        string sql = "SELECT v_BudgetForecastYear.AccountNumber,v_BudgetForecastYear.YearDays, v_BudgetForecastYear.AccountName, ROUND(ISNULL(v_BudgetForecastYear.ForeCast, 0), 0) AS Budget, " +
                   "v_BudgetForecastYear.ForeCast,v_BudgetForecastYear.YearDays," +
                   "Comment=isnull((select YearComment from [dbo].v_BudgetForecastYear V1 where V1.CoCode='" + Comp.Substring(0, 3) + "' AND V1.Vess='" + vess.Substring(0, 3) + "' and V1.AcctId=v_BudgetForecastYear.AcctId and V1.CurFinYear='" + CurrFinancialYr.ToString() + "'),''), " +
                   "v_BudgetForecastYear.MidCatID, v_BudgetForecastYear.MinCatID, " +
                   "    (SELECT     MidCat " +
                   "      FROM          dbo.tblAccountsMid AS G2 " +
                   "      WHERE      (MidCatID = v_BudgetForecastYear.MidCatID)) AS Group1, " +
                   ...
                   "    ((SELECT     Amount " +
                   "        FROM         dbo.Add_v_BudgetForecastYear AS Addt " +
                   "        WHERE     (CoCode = v_BudgetForecastYear.CoCode) AND (AcctId = v_BudgetForecastYear.AcctID) AND (CurFinYear = '" + CurrFinancialYr.ToString() + "')), 0) AS AnnAmt,  " +
                   "v_BudgetForecastYear.YearComment as Comment," +
                   "v_BudgetForecastYear.Year, v_BudgetForecastYear.ForeCastYear,MajSeqNo,MidSeqNo,MinSeqNo,v_BudgetForecastYear.accountnumber,accountid " +
                   "FROM         dbo.v_BudgetForecastYear AS v_BudgetForecastYear INNER JOIN " +
                   "    (SELECT     RESULT AS CY " +
                   "FROM dbo.CSVtoTableStr('" + CurrFinancialYr.ToString() + "', ',') AS CSVtoTableStr_1) AS tempYear ON v_BudgetForecastYear.CurFinYear = tempYear.CY WHERE v_BudgetForecastYear.COCODE='" + Comp.Substring(0, 3) + "' AND v_BudgetForecastYear.VESS='" + vess.Substring(0, 3) + "' AND CurFinYear='" + CurrFinancialYr.ToString()+"'";
        ...
        DataTable dtAccts1 = Common.Execute_Procedures_Select_ByQuery("select * from ( select distinct midcatid,midcat,MajSeqNo,MidSeqNo from dbo.v_New_CurrYearBudgetHome WHERE MajCatId<>6 and MidSeqNo <= 170) A ORDER BY MajSeqNo,MidSeqNo");
        DataTable dtDays = Common.Execute_Procedures_Select_ByQuery("select top 1 Days From dbo.v_New_CurrYearBudgetHome Where shipid='" + vess.Substring(0, 3) + "' AND CurFinYear='" + CurrFinancialYr.ToString() + "' order by days desc");
        int DaysCnt = 1;
        if (dtDays != null)
        {
            if (dtDays.Rows.Count > 0)
        ...
        {
            int RowSum = 0;
            DataTable dtShip = Common.Execute_Procedures_Select_ByQuery("select budget from dbo.v_New_CurrYearBudgetHome where shipid='" + vess.Substring(0, 3) + "' AND CurFinYear='" + CurrFinancialYr.ToString() + "' and  midcatid=" + dtAccts.Rows[i][0].ToString());
            if (dtShip != null)
            {
                if (dtShip.Rows.Count > 0)
                {
        ...
        for (int i = 0; i <= dtAccts1.Rows.Count - 1; i++)
        {
            DataTable dtShip = Common.Execute_Procedures_Select_ByQuery("select budget from dbo.v_New_CurrYearBudgetHome where shipid='" + vess.Substring(0, 3) + "' AND CurFinYear='" + CurrFinancialYr.ToString() + "' and  midcatid=" + dtAccts1.Rows[i][0].ToString());
            if (dtShip != null)
            {
                if (dtShip.Rows.Count > 0)
                {
        ...
        rpt.SetParameterValue("Start Date", StartDate);
        rpt.SetParameterValue("End Date", EndDate);
        rpt.SetParameterValue("Year", CurrFinancialYr);
        rpt.SetParameterValue("Days", Days);
    ...
    public void SHOW_AccountMapping()
    {
        ...
        CrystalReportViewer1.Visible = true;
        CrystalReportViewer1.ReportSource = rpt;
        rpt.Load(Server.MapPath("~/Modules/OPEX/Report/AccountMappingReport.rpt"));
    ...
    public void CurrentYearProjection()
    {
        ...
        CrystalReportViewer1.ReportSource = rpt;
        rpt.Load(Server.MapPath("~/Modules/OPEX/Report/CurrentYearProjection1.rpt"));
        rpt.SetDataSource(dt);
