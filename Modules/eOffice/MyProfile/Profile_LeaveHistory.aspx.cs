...
public partial class emtm_MyProfile_Emtm_Profile_LeaveHistory : System.Web.UI.Page
{
    DateTime ToDay;
    public AuthenticationManager auth; //int EmpId = Common.CastAsInt32(Session["ProfileId"]);
    ...
    private void BindLeavesGrid(string Month, string LeaveType)
    {
        ...
        if (Month == "0")
        {
            ...
            " a.Reason,case a.Status when 'A' then case when convert(datetime,a.LeaveTo) < convert(datetime,convert(varchar,getdate(),106))  then 'T' else 'A' end " +
            " else a.Status end as StatusCode,case a.Status when 'A'  then case when convert(datetime,a.LeaveTo) < convert(datetime,convert(varchar,getdate(),106))  then 'Taken' else 'Approved' end " +
            " when 'P' then 'Plan' when 'V' then 'Awaiting Approval' when 'R' then 'Rejected' when 'C' then 'Cancelled' else a.Status end as Status,a.VerifiyRemarks, " +
            " (case when a.HalfDay>0 then 0.5 else dbo.HR_Get_LeaveCount(" + ViewState["OfficeId"].ToString() + ",leavefrom,leaveto) end) as Duration " +
    ...
    {
        ScriptManager.RegisterStartupScript(Page, this.GetType(), "success", "PopUPLeaveStatusPrintWindow('" + Common.CastAsInt32(ddlCurrentYear.SelectedValue) + "','" + EmpId + "','" + ViewState["OfficeId"] + "');", true);
    }
    #endregion
}