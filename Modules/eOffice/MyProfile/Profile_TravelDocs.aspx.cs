...
public partial class Emtm_Profile_TravelDocs : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        int chpageauth = UserPageRelation.CheckUserPageAuthority(Convert.ToInt32(Session["loginid"].ToString()), 259);
        if (chpageauth <= 0)
        {
            Response.Redirect("../AuthorityError.aspx");
        }
        //*******************
        auth = new AuthenticationManager(259, Common.CastAsInt32(Session["loginid"]), ObjectType.Page);
    ...
    private void BindGrid()
    {
        ...
        if (EmpId > 0)
        {
            DataTable dt = Common.Execute_Procedures_Select_ByQueryCMS("select TravelDocId,EmpId,DocumentNo,case when ECNR=1 then 'Yes' else 'No' end as ECNR,replace(convert(varchar,IssueDate,106),' ','-') as IssueDate,replace(convert(varchar,ExpiryDate,106),' ','-') as ExpiryDate ,PlaceofIssue,[FileName],FileImage from HR_TravelDocumentDetails WHERE  DocsCategory='P' and EMPID=" + EmpId.ToString());
        ...
        if (EmpId > 0)
        {
            ...
            if (dtdata != null)
                if (dtdata.Rows.Count > 0)
                {
                    ...
                    txtDocno.Text = dr["DocumentNo"].ToString().Trim();
                    ...
                    ChkECNR.Checked =Convert.ToBoolean(dr["ECNR"]); 
                }
        }
    }
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
            ...
            Common.Set_Parameters(new MyParameter("@TravelDocId", SelectedId),
                ...
                new MyParameter("@DocumentNo", txtDocno.Text.Trim()),
                new MyParameter("@ECNR", ChkECNR.Checked),
                new MyParameter("@Country", ""),
                ...
                new MyParameter("@DocsCategory","P"),
