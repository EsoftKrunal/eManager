...
public partial class Emtm_Profile_MedicalCertificateDocs : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        int chpageauth = UserPageRelation.CheckUserPageAuthority(Convert.ToInt32(Session["loginid"].ToString()), 264);
        if (chpageauth <= 0)
        {
            Response.Redirect("../AuthorityError.aspx");
        }
        //*******************
        auth = new AuthenticationManager(264, Common.CastAsInt32(Session["loginid"]), ObjectType.Page);

        Session["CurrentModule"] = 3;
        if (!Page.IsPostBack)
        {
            ControlLoader.LoadControl(ddlMedicalDocName, DataName.HR_Medical, "Select", "");
    ...
    private void BindGrid()
    {
        ...
        if (EmpId > 0)
        {
            DataTable dt = Common.Execute_Procedures_Select_ByQueryCMS("select D.MedicalDocId,D.EmpId,M.MedicalDocumentName as DocumentName,D.DocumentNo,D.PlaceOfIssue,replace(convert(varchar,D.IssueDate,106),' ','-') as IssueDate,replace(convert(varchar,D.ExpiryDate,106),' ','-')as ExpiryDate,[FileName],FileImage from HR_MedicalDocsDetails D left outer join HR_Medical M on D.DocumentNameId=M.MedicalDocumentId where EmpId=" + EmpId.ToString());
            RptMedicalDoc.DataSource = dt;
            RptMedicalDoc.DataBind();
        }
    }
    public void ShowRecord(int Id)
    {
        ...
        if (EmpId > 0)
        {
            DataTable dtdata = Common.Execute_Procedures_Select_ByQueryCMS("select * from HR_MedicalDocsDetails WHERE MedicalDocId=" + Id.ToString());
            if (dtdata != null)
                if (dtdata.Rows.Count > 0)
                {
                    DataRow dr = dtdata.Rows[0];
                    ddlMedicalDocName.SelectedValue = dr["DocumentNameId"].ToString().Trim();
    ...
    {
        ddlMedicalDocName.SelectedIndex = 0;
        txtDocumentNo.Text = "";
    ...
    protected void btndocDelete_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (EmpId > 0)
        {
            try
            {
                DataTable dt = Common.Execute_Procedures_Select_ByQueryCMS("delete from HR_MedicalDocsDetails where MedicalDocId=" + SelectedId);
                BindGrid();
                setButtons("Cancel"); 
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
            ...
            int EmpId = Common.CastAsInt32(Session["ProfileId"]);
            Common.Set_Procedures("HR_InsertUpdateMedicalDocsDetails");
            Common.Set_ParameterLength(9);
            Common.Set_Parameters(new MyParameter("@MedicalDocId", SelectedId),
                new MyParameter("@EmpId", EmpId),
                new MyParameter("@DocumentNameId", ddlMedicalDocName.SelectedValue.Trim()),
