...
public partial class Emtm_OfficeAbsenceHotoApproval_L : System.Web.UI.Page
{
    ...
    protected void btnApprove_Onclick(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["key"].Trim().EndsWith("B"))
            sql = "Update DBO.HR_OfficeAbsence_HOTOMaster_L set BStatus='A' , BOn=Getdate() Where HotoID=" + HotoID.ToString();
        else if (Request.QueryString["key"].Trim().EndsWith("H"))
            sql = "Update DBO.HR_OfficeAbsence_HOTOMaster_L set HStatus='A' , HOn=Getdate() Where HotoID=" + HotoID.ToString();
        else
            return;
    ...
    protected void btnReject_Onclick(object sender, EventArgs e)
    {
        string sql = "";
        if (Request.QueryString["key"].Trim().EndsWith("B"))
            sql = "Update DBO.HR_OfficeAbsence_HOTOMaster_L set BStatus='R' , BOn=Getdate() Where HotoID=" + HotoID.ToString();
        else if (Request.QueryString["key"].Trim().EndsWith("H"))
            sql = "Update DBO.HR_OfficeAbsence_HOTOMaster_L set HStatus='R' , HOn=Getdate() Where HotoID=" + HotoID.ToString();
        else
            return;
    ...
    {
        string SQL = "SELECT REPLACE(Convert(varchar(11), OA.LeaveFrom, 106),' ','-') AS LeaveFrom, REPLACE(Convert(varchar(11), OA.LeaveTo, 106),' ','-') AS LeaveTo FROM HR_LeaveRequest OA " +
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            DataTable dtHoto = Common.Execute_Procedures_Select_ByQueryCMS("SELECT (Select FirstName +' '+FamilyName from Hr_PersonalDetails PD Where PD.EmpID =HT.HandOverToID)HandOverTo,(Select FirstName +' '+FamilyName from Hr_PersonalDetails PD Where PD.EmpID =HT.BackupId)BackupBy,* FROM HR_OfficeAbsence_HOTOMaster_L HT where HotoId=" + HotoID);
