...
public partial class emtm_Emtm_Profile_LeaveStatus : System.Web.UI.Page
{
    DateTime ToDay;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblCurYear4.Text = ddlCurrentYear.SelectedValue;
        }
    }
    //-----------------------
    ...
    private void BindGrid()
    {
        int EmpId = Common.CastAsInt32(Session["ProfileId"]);
            if (EmpId > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ViewState["AnnualLeave"] = Common.CastAsInt32(dt.Rows[0]["LeaveCount"]);
    ...
    public void ShowRecord(int Id)
    {
        ...
        if (EmpId > 0)
        {
            ...
            //           "( " +
            //           "SELECT a.EmpCode,a.firstname,a.middlename,a.familyname,a.Office,a.Department,c.OfficeName,a.Position,d.DeptName,b.PositionName, " +
            ...
            if(dt!=null )
                if (dt.Rows.Count > 0)
                {
                    ...
                    lblLeaveBalance.Text = dr["BalLeaveCurr"].ToString();
    ...
    public double SRound(double Real)
    {
    ...
    private void BindLeavesGrid()
    {
        ...
        string sql = "select a.LeaveRequestId,a.EmpId,a.LeaveTypeId,b.LeaveTypeName,replace(convert(varchar,a.LeaveFrom,106),' ','-') as LeaveFrom, " +
                    " replace(convert(varchar,a.LeaveTo,106),' ','-') as LeaveTo," +
    ...
    private void BindLeavesGrid(string Month,string LeaveType )
    {
        ...
        if (Month != "0")
        {
            sql = "select a.LeaveRequestId,a.EmpId,a.LeaveTypeId,b.LeaveTypeName,replace(convert(varchar,a.LeaveFrom,106),' ','-') as LeaveFrom, " +
                        ...
                        " where EmpId=" + EmpId + " and " + Month + " between month(LeaveFrom) and month(LeaveTO) and year(LeaveFrom)=" + ddlCurrentYear.SelectedValue + " and a.LeaveTypeId=" + LeaveType + " order by a.LeaveFrom desc";
        ...
        else
        {
            sql = "select a.LeaveRequestId,a.EmpId,a.LeaveTypeId,b.LeaveTypeName,replace(convert(varchar,a.LeaveFrom,106),' ','-') as LeaveFrom, " +
                       ...
                       " (case when a.HalfDay>0 then 0.5 else dbo.HR_Get_LeaveCount(" + ViewState["OfficeId"].ToString() + ",leavefrom,leaveto) end) as Duration " +
                       " from HR_LeaveRequest a left outer join HR_LeaveTypeMaster b on a.LeaveTypeId=b.LeaveTypeId " +
                       " where EmpId=" + EmpId + " and year(LeaveFrom)=" + ddlCurrentYear.SelectedValue + " and a.LeaveTypeId=" + LeaveType + " order by a.LeaveFrom desc";
        }
    ...
    {
        SelectedId = 0;
        ScriptManager.RegisterStartupScript(Page, this.GetType(), "success", "PopUPWindow('" + SelectedId + "'," + ViewState["OfficeId"] + "," + ViewState["DepartmentId"] + ",'A');", true);
    }
    protected void btnhdn_Click(object sender, EventArgs e)
    {
        SelectedId = 0; 
        BindGrid();
        //btnShowLeaves_Click(sender, e);
        BindLeavesGrid();
    }
    protected void btnLeaveEdit_Click(object sender, ImageClickEventArgs e)
    {
        SelectedId = Common.CastAsInt32(((ImageButton)sender).CommandArgument);
        ScriptManager.RegisterStartupScript(Page, this.GetType(), "success", "PopUPWindow('" + SelectedId + "'," + ViewState["OfficeId"] + "," + ViewState["DepartmentId"] + ",'E');", true);
    }
    protected void btnLeaveCancel_Click(object sender, ImageClickEventArgs e)
    {
        int EmpId = Common.CastAsInt32(Session["ProfileId"]);
        SelectedId = Common.CastAsInt32(((ImageButton)sender).CommandArgument);
        Common.Set_Procedures("HR_Cancel_LeaveRequest");
        Common.Set_ParameterLength(2);
        Common.Set_Parameters(new MyParameter("@LeaveRequestId", SelectedId),
            new MyParameter("@CancelledBy", EmpId));
        DataSet ds = new DataSet();
        if (Common.Execute_Procedures_IUD_CMS(ds))
        {
            BindLeavesGrid();
            ScriptManager.RegisterStartupScript(Page, this.GetType(), "success", "alert('Leave cancelled successfully.');", true);
        }
        else
        {
            ScriptManager.RegisterStartupScript(Page, this.GetType(), "success", "alert('Unable to cancel leave.');", true);
        }
    }
    ...
    {
        ScriptManager.RegisterStartupScript(Page, this.GetType(), "success", "PopUPLeaveStatusPrintWindow('" + Common.CastAsInt32(ddlCurrentYear.SelectedValue) + "','" + Common.CastAsInt32(Session["ProfileId"]) + "');", true);
    }
    #endregion
}
