...
public partial class Emtm_Profile_OtherDocs : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        if (EmpId > 0)
        {
            DataTable dt = Common.Execute_Procedures_Select_ByQueryCMS("select D.OtherDocId,D.EmpId,D.DocumentName,D.DocumentNo,D.PlaceOfIssue,replace(convert(varchar,D.IssueDate,106),' ','-') as IssueDate,replace(convert(varchar,D.ExpiryDate,106),' ','-')as ExpiryDate,[FileName],FileImage from HR_OtherDocsDetails D where EmpId=" + EmpId.ToString());
    ...
    {
        DateTime date_Issue=DateTime.Today, date_Expiry;
    ...
    protected void btndocDelete_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (EmpId > 0)
        {
            try
            {
                DataTable dt = Common.Execute_Procedures_Select_ByQueryCMS("delete from HR_OtherDocsDetails where OtherDocId=" + SelectedId);
                BindGrid();
                setButtons("Cancel"); 
