...
public partial class Emtm_Profile_VisaDocs : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        int chpageauth = UserPageRelation.CheckUserPageAuthority(Convert.ToInt32(Session["loginid"].ToString()), 260);
        if (chpageauth <= 0)
        {
            Response.Redirect("../AuthorityError.aspx");
        }
        //*******************
        auth = new AuthenticationManager(260, Common.CastAsInt32(Session["loginid"]), ObjectType.Page);
        ...
        string EmpMode = Session["ProfileMode"].ToString();
            ...
            if (EmpMode == "Edit")
            {
                switch (Action)
                {
                    case "View":
                        tblview.Visible = true;
                        divTraveldoc.Style.Add("height", "180px;");
    ...
    private void BindGrid()
    {
        ...
        if (EmpId > 0)
        {
            DataTable dt = Common.Execute_Procedures_Select_ByQueryCMS("select a.TravelDocId,a.EmpId,a.DocumentNo,b.CountryName,a.VisaType,replace(convert(varchar,a.IssueDate,106),' ','-') as IssueDate,replace(convert(varchar,a.ExpiryDate,106),' ','-') as ExpiryDate ,a.PlaceofIssue,a.[FileName],a.FileImage from HR_TravelDocumentDetails a inner join Country b on a.country= b.countryId WHERE  DocsCategory='V' and EMPID=" + EmpId.ToString());
            RptTravelVisa.DataSource = dt;
            RptTravelVisa.DataBind();
        }
    }
        ...
        if (EmpId > 0)
        {
            ...
            if (dtdata != null)
                if (dtdata.Rows.Count > 0)
                {
                    ...
                    txtVisaType.Text = dr["VisaType"].ToString().Trim();  
                    txtVisano.Text = dr["DocumentNo"].ToString().Trim();
