...
public partial class emtm_MyProfile_Emtm_PopupLeaveRequest : System.Web.UI.Page
{
    ...
    public void ShowDateConflictMessage()
    {
        if (txtLeaveFrom.Text.Trim() != "" && txtLeaveTo.Text.Trim() != "")
        {
            ...
            if(dTuser.Rows.Count >0)
            {
                ...
                {
                    litNotes.Text += "<br/> ■ There is Date Conflict between Leave & Vetting Plan.";
                }
            }
        }
    }
...
//        string WhereClause = "where a.Status <>'R' and pd.status<>'I' and ofice.officeid=" + Common.CastAsInt32(ddlLocation.SelectedValue.Trim());
  ...
  string SqlEmpName = "select pd.empid,pd.FirstName +' ' + pd.MiddleName +' ' + pd.FamilyName as [Name] " +
        ...
        if (Common.CastAsInt32(ddlUsers.SelectedValue) > Common.CastAsInt32(0))
        {
            ...
            if (dtGetEmployeeInfo != null)
                if (dtGetEmployeeInfo.Rows.Count > 0)
                {
                    ...
                    //string[] CCAdds = { "pankaj.k@esoftech.com" };// MailTo.ToString().Split(Sep);
    ...
    public string SendMailtoApproverChain()
    {
        ...
        DataTable dtApproverChain = Common.Execute_Procedures_Select_ByQueryCMS("EXEC dbo.Emtm_LeaveDiscussion_getApproverChain " + LeaveRequestId);
        if (dtApproverChain.Rows.Count >0)
        {
            ...
            if (dtGetEmployeeInfo != null)
                if (dtGetEmployeeInfo.Rows.Count > 0)
                {
                    ...
                    //MailFrom = "asingh@energiossolutions.com";
                    //string[] ToAdds = "asingh@energiossolutions.com".ToString().Split(Sep);
    ...
    {        
        ...
        if (ddlOffice.SelectedValue.Trim() != "")
        {
            ...
            {
                Position = "29,64,83,116,66,67,128";
            }
            ...
            {
                Position = "1,4,89,16,100,113,5,57,152";
            }
            if (ddlOffice.SelectedValue.Trim() == "4")
            {
                Position = "71,78";
            }
	    if (ddlOffice.SelectedValue.Trim() == "5")
            {
		Position = "1";
            }
        }
        ...
        string Query = "SELECT FU.* FROM ForwardedUsers FU " +
                       "INNER JOIN Hr_PersonalDetails PD ON PD.EmpId = FU.EmpId AND PD.DRC IS NULL " +
	...
	protected void LoadApproverChain()
	{
		DataTable dtForwaredeUsers = Common.Execute_Procedures_Select_ByQueryCMS("select * from DBO.GET_REPORTING_CHAIN(" + EmpId + ")");

        ddlUsers.DataSource = dtForwaredeUsers;
        ddlUsers.DataTextField = "EmpName";
