...
public partial class emtm_StaffAdmin_Emtm_Popup_BusinessTripDetails : System.Web.UI.Page
{
    DateTime ToDay;
    public int Emp_Id
    {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            {                
                ...
                int EmpId = Common.CastAsInt32(Request.QueryString["EmpId"].ToString());
    ...
    {
        string sql = "select a.LeaveRequestId,a.EmpId,replace(convert(varchar,a.LeaveFrom,106),' ','-') as LeaveFrom,replace(convert(varchar,a.LeaveTo,106),' ','-') as LeaveTo, a.Reason,replace(convert(varchar,a.EstimatedReturnDt,106),' ','-') as EstimatedReturnDt," +
                     "CASE WHEN a.Location = 1 THEN 'Local' WHEN a.Location = 2 THEN 'Internation' Else ' ' END AS Location,(case when a.HalfDay>0 then 0.5 else DateDiff(d,leavefrom,leaveto+1) end) as Duration " +
                     "from HR_OfficeAbsence a " +
                     "left outer join HR_LeavePurpose b on a.PurposeId=b.PurposeId " +
                     "where year(a.Leavefrom)=" + ViewState["CurrentYear"].ToString().Trim() + " and a.EmpId=" + Emp_Id + "  and year(a.LeaveTo)=" + ViewState["CurrentYear"].ToString().Trim() + " order by a.LeaveFrom desc";
