...
public partial class emtm_Emtm_TravelDocs : System.Web.UI.Page
{
        ...
        string EmpMode = Session["EmpMode"].ToString();
            ...
            if (EmpMode == "Edit")
            {
                switch (Action)
                {
                    case "View":
                        tblview.Visible = true;
                        divTraveldoc.Style.Add("height", "260px;");
                    ...
                    default:
                        tblview.Visible = false;
                        divTraveldoc.Style.Add("height", "365px;");
    ...
    private void BindGrid()
    {
        ...
        if (EmpId > 0)
        {
            DataTable dt = Common.Execute_Procedures_Select_ByQueryCMS("select FinNricId,EmpId,Fin_Nric_No,Fin_Nric_Type,replace(convert(varchar,IssueDate,106),' ','-') as IssueDate,replace(convert(varchar,ExpiryDate,106),' ','-') as ExpiryDate,FileName,FileImage from HR_FinNricDetails WHERE  EMPID=" + EmpId.ToString());
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
        ...
        FileUpload img = (FileUpload)flFinNricdocument;
        Byte[] imgByte = new Byte[0];
        string FileName = "";
        if (img.HasFile && img.PostedFile != null)
        {
            HttpPostedFile File = flFinNricdocument.PostedFile;
            FileName = flFinNricdocument.FileName.Trim();
